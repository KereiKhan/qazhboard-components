<div x-data="select({ data: {{ $items }}, emptyOptionsMessage: '{{ $emptyOptionsMessage }}', name: '{{ $name }}', placeholder: '{{ $placeholder }}' })" x-init="init()" @click.away="closeListbox()" @keydown.escape="closeListbox()"
  class="relative">
  <span class="inline-block w-full rounded-md">
    <button x-ref="button" type="button" @click="toggleListboxVisibility()" :aria-expanded="open" aria-haspopup="listbox"
      class="relative z-0 w-full py-2.5 pl-5 pr-10 text-left transition duration-150 ease-in-out bg-white border border-gray-300 rounded-md cursor-pointer focus:outline-none focus:shadow-outline-blue focus:border-emerald-400 sm:text-sm sm:leading-5 dark:bg-slate-900 dark:border-gray-700">
      <span x-show="! open" x-text="value in options ? options[value] : placeholder"
        :class="!(value in options) ? 'text-gray-400' : 'text-slate-900 dark:text-white'" class="block truncate"></span>

      <input x-ref="search" x-show="open" x-model="search" @keydown.enter.stop.prevent="selectOption()"
        @keydown.arrow-up.prevent="focusPreviousOption()" @keydown.arrow-down.prevent="focusNextOption()" type="search"
    ...
    <ul x-ref="listbox" @keydown.enter.stop.prevent="selectOption()" @keydown.arrow-up.prevent="focusPreviousOption()"
      ...
      <template x-for="(key, index) in Object.keys(options)" :key="index">
        <li :id="name + 'Option' + focusedOptionIndex" @click="selectOption()" @mouseenter="focusedOptionIndex = index"
          ...
          class="relative py-2 pl-3 text-gray-900 cursor-default select-none pr-9 dark:text-white">
