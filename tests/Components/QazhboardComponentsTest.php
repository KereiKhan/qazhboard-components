...
test('it can register assets', function () {
    ...
    $this->assertStringContainsString('https://unpkg.com/filepond@^4/dist/filepond.css', QazhboardComponents::outputStyles());
});

test('it can see output styles', function () {
    $styles = QazhboardComponents::outputStyles();

    $this->assertStringContainsString('https://unpkg.com/filepond@^4/dist/filepond.css', $styles);
});

test('it can see output scripts', function () {
    $scripts = QazhboardComponents::outputScripts();

    $this->assertStringContainsString('https://cdnjs.cloudflare.com/ajax/libs/alpinejs/2.3.0/alpine-ie11.js', $scripts);
});
...
HTML;
    ...
    $this->blade($template)
        ...
        ->assertSee('https://cdnjs.cloudflare.com/ajax/libs/alpinejs/2.3.0/alpine-ie11.js');
});
